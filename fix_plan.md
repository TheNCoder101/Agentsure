...
- [x] Receipt persistence + `GET /receipts/{id}` lookup + CSV/PDF export (Growth plan retention promise); SQLite -> Postgres migration path. _(done 2026-07-11: new `receipts` table in app/accounts/store.py, scoped by `key_id` — a key can only read back its own receipts (404, not 403, on cross-key lookup to avoid existence leaks). Persists only the Receipt model's own hash/metadata fields, never raw output/source text, matching the receipt's existing privacy-conscious design. `POST /verify` now calls `store.save_receipt`. Added `GET /receipts` (list, `?format=csv` for bulk export), `GET /receipts/{id}` (lookup), `GET /receipts/{id}/pdf` (single-receipt PDF via app/receipt/export.py, using fpdf2 rather than hand-rolled PDF binary format — not worth the byte-offset-correctness risk for a one-page document). Postgres migration path unchanged/deferred, same as before — SQLite is fine until there's a reason to move. 12 new tests, 78 total, ruff+mypy clean.)_
...
- [x] Cheap-pass polish: negation-polarity blindness, lexical-only matching, per-claim retokenization. _(done 2026-07-11: added a dedicated negation surface check in app/verify/cheap.py — "not"/"no" used to be stopwords, so a negated claim could score as supported against an affirmative source with no way to catch it; removed them from the stopword list and added a check comparing negation markers between the claim and its closest-matching source sentence, only firing when that sentence match is confident. Added lossy suffix-stripping stemming to `_tokens()` so tense/inflection variants overlap (increase/increased/increases/increasing). Precomputed source tokens/sentences once per `/verify` call (`_index_sources`) instead of re-tokenizing per claim; `HeuristicJudge` now shares the same sentence-splitter instead of duplicating it. Investigated the previously-proposed "short-claim threshold volatility" fix and dropped it — the ratio math already self-corrects (a 3-token claim needs 2/3 overlap to clear 0.55, effectively requiring near-full overlap for short claims already), so there was no real bug there. 6 new tests, 66 total, ruff+mypy clean.)_
...
- 2026-07-11 — V2 Phase 0 cheap-pass polish (user-directed) — see backlog entry above for detail; 66 tests pass, ruff+mypy clean — next: receipt persistence + export (Phase 0 item 2).
- 2026-07-11 — V2 Phase 0 item 2, receipt persistence + export (user-directed, continuing the loop) — see backlog entry above for detail; 78 tests pass, ruff+mypy clean — next: Ed25519 asymmetric signing (Phase 0 item 3), so receipts are verifiable offline by a third party instead of only via our own /receipt/verify.
